#!/usr/bin/env python3
"""
Test 8: Multiple Independent Branches
Three independent computation branches with different lengths.
A->B, C->D->E, F->G
Expected: Tests horizontal fusion across multiple branches of different depths.
"""

import os
import sys

sys.path.insert(0, "/home/sifi/pykokkos-fuse")

import pykokkos as pk


@pk.workunit
def init_A(i: int, A: pk.View1D[float], val: float):
    A[i] = val


@pk.workunit
def compute_B(i: int, A: pk.View1D[float], B: pk.View1D[float]):
    B[i] = A[i] * 2.0


@pk.workunit
def init_C(i: int, C: pk.View1D[float], val: float):
    C[i] = val


@pk.workunit
def compute_D(i: int, C: pk.View1D[float], D: pk.View1D[float]):
    D[i] = C[i] * 2.0


@pk.workunit
def compute_E(i: int, D: pk.View1D[float], E: pk.View1D[float]):
    E[i] = D[i] + 5.0


@pk.workunit
def init_F(i: int, F: pk.View1D[float], val: float):
    F[i] = val


@pk.workunit
def compute_G(i: int, F: pk.View1D[float], G: pk.View1D[float]):
    G[i] = F[i] * 3.0


def run_test():
    N = 1000
    A = pk.View([N], dtype=float)
    B = pk.View([N], dtype=float)
    C = pk.View([N], dtype=float)
    D = pk.View([N], dtype=float)
    E = pk.View([N], dtype=float)
    F = pk.View([N], dtype=float)
    G = pk.View([N], dtype=float)

    pk.parallel_for(N, init_A, A=A, val=1.0)
    pk.parallel_for(N, compute_B, A=A, B=B)
    pk.parallel_for(N, init_C, C=C, val=2.0)
    pk.parallel_for(N, compute_D, C=C, D=D)
    pk.parallel_for(N, compute_E, D=D, E=E)
    pk.parallel_for(N, init_F, F=F, val=3.0)
    pk.parallel_for(N, compute_G, F=F, G=G)

    pk.flush()

    assert A[0] == 1.0, f"A[0] = {A[0]}, expected 1.0"
    assert B[0] == 2.0, f"B[0] = {B[0]}, expected 2.0"
    assert C[0] == 2.0, f"C[0] = {C[0]}, expected 2.0"
    assert D[0] == 4.0, f"D[0] = {D[0]}, expected 4.0"
    assert E[0] == 9.0, f"E[0] = {E[0]}, expected 9.0"
    assert F[0] == 3.0, f"F[0] = {F[0]}, expected 3.0"
    assert G[0] == 9.0, f"G[0] = {G[0]}, expected 9.0"

    return True


if __name__ == "__main__":
    success = run_test()
    print(f"Test 8: {'PASSED' if success else 'FAILED'}")
